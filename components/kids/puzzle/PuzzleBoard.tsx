...
function WallArt({ world }: { world: WorldId }) {
  if (world === "lagoon") {
    return (
      <svg viewBox="0 0 40 40" className={styles.art} aria-hidden="true">
        <path d="M4 15q4-4 8 0t8 0 8 0 8 0" fill="none" stroke="var(--text)" strokeWidth="2.6" strokeLinecap="round" />
        <path d="M4 27q4-4 8 0t8 0 8 0 8 0" fill="none" stroke="var(--text)" strokeWidth="2.6" strokeLinecap="round" />
      </svg>
    );
  }
  if (world === "caves") {
    return (
      <svg viewBox="0 0 40 40" className={styles.art} aria-hidden="true">
        <polygon points="5,35 11,16 17,21 21,5 29,18 35,35" fill="var(--accent)" stroke="var(--accent)" strokeWidth="2.2" strokeLinejoin="round" />
        <polygon points="21,5 24,20 20,35 17,21" fill="var(--accent)" opacity="0.9" />
  ...
  return (
    <svg viewBox="0 0 40 40" className={styles.art} aria-hidden="true">
      <circle cx="13" cy="25" r="10" fill="var(--accent)" stroke="var(--accent)" strokeWidth="2" />
      <circle cx="27" cy="25" r="10" fill="var(--accent)" stroke="var(--accent)" strokeWidth="2" />
      <circle cx="20" cy="16" r="11" fill="var(--accent)" stroke="var(--accent)" strokeWidth="2" />
      <circle cx="15" cy="14" r="2.4" fill="var(--warn)" />
      <circle cx="25" cy="21" r="2.4" fill="var(--error)" />
    </svg>
  );
}
...
function FlagArt({ waving }: { waving: boolean }) {
  return (
    <svg viewBox="0 0 40 40" className={styles.art} aria-hidden="true">
      <ellipse cx="18" cy="34" rx="11" ry="3.5" fill="color-mix(in srgb, var(--surface-hi) 20%, transparent)" />
      <rect x="12" y="5" width="3.2" height="29" rx="1.6" fill="var(--surface-hi)" />
      <path d="M15 6h17l-5 6.5 5 6.5H15z" fill="var(--error)" stroke="var(--surface-hi)" strokeWidth="2" strokeLinejoin="round" className={waving ? styles.flagWave : undefined} />
    </svg>
  );
}
...
function GemArt({ collected }: { collected: boolean }) {
  return (
    <svg viewBox="0 0 40 40" className={`${styles.art} ${styles.gem} ${collected ? styles.gemGone : ""}`} aria-hidden="true">
      <path d="M12 9h16l6 8-14 16L6 17z" fill="var(--error)" stroke="var(--surface-hi)" strokeWidth="2.2" strokeLinejoin="round" />
...
function RobotToken() {
  return (
    <svg viewBox="0 0 40 40" className={styles.token} aria-hidden="true">
      <rect x="4.5" y="13" width="6" height="16" rx="2.5" fill="var(--text-dim)" stroke="var(--surface-hi)" strokeWidth="1.8" />
      <rect x="29.5" y="13" width="6" height="16" rx="2.5" fill="var(--text-dim)" stroke="var(--surface-hi)" strokeWidth="1.8" />
      <path d="M20 9V4" stroke="var(--surface-hi)" strokeWidth="2.2" strokeLinecap="round" />
      <circle cx="20" cy="3.6" r="2.6" fill="var(--warn)" stroke="var(--surface-hi)" strokeWidth="1.6" />
      <rect x="9" y="8.5" width="22" height="26" rx="9" fill="var(--warn)" stroke="var(--surface-hi)" strokeWidth="2.4" />
      <rect x="11.5" y="10.5" width="17" height="11" rx="5" fill="var(--surface-hi)" />
      <circle cx="16.4" cy="16" r="2.4" fill="var(--accent)" />
      <circle cx="23.6" cy="16" r="2.4" fill="var(--accent)" />
      <path d="M15.5 28h9" stroke="var(--warn)" strokeWidth="2.6" strokeLinecap="round" />
    </svg>
  );
}
